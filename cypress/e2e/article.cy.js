...
describe('Conduit', () => {
  ...
  it('should allows logged in user to create an article', () => {
    cy.login(user.email, user.username, user.password);
    cy.reload();
    cy.contains('.nav-link', 'New Article').click();
    cy.getPlaceholder('Article Title').type(article.title);
    cy.getPlaceholder('What\'s this').type(article.description);
    cy.getPlaceholder('Write your').type(article.body);
    cy.contains('button', 'Publish Article').click();
    cy.get('h1').should('contain', article.title);
    cy.get('.col-md-12').should('contain', article.body);
  });

  it('should allows logged in user to delete his article', () => {
    cy.login(user.email, user.username, user.password);
    cy.reload();
